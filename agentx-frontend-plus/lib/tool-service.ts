...
export async function deleteMarketTool(toolId: string): Promise<ApiResponse<any>> {
  try {
    return await httpClient.delete(`/tools/market/${toolId}`);
  } catch (error) {
    return {
      code: 500,
      message: error instanceof Error ? error.message : "下架工具失败",
      data: null,
      timestamp: Date.now()
    };
  }
...
export const deleteMarketToolWithToast = withToast(deleteMarketTool, {
  successTitle: "下架成功",
  errorTitle: "下架失败",
  showSuccessToast: true,
  showErrorToast: true,
