...
import { Search, X, Plus, Wrench, Download, Info, User, Check, ChevronRight, ArrowLeft, Trash2 } from "lucide-react"
...
import { deleteMarketToolWithToast, getMarketToolsWithToast, installToolWithToast } from "@/lib/tool-service"
...
export default function ToolsMarketPage() {
  ...
  const handleDeleteMarketTool = async (tool: Tool & { isOwner?: boolean }) => {
    if (!window.confirm(`确定要从工具市场下架 "${tool.name}" 吗？`)) {
      return;
    }

    const actualToolId = tool.toolId || tool.id;
    const response = await deleteMarketToolWithToast(actualToolId);
    if (response.code === 200) {
      setTools(prev => prev.filter(item => (item.toolId || item.id) !== actualToolId));
    }
  }
            ...
            <Card key={tool.id} className="group relative overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-background/80 opacity-0 transition-opacity group-hover:opacity-100">
                <div className="flex gap-2">
                  ...
                  {(tool as Tool & { isOwner?: boolean }).isOwner && (
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => handleDeleteMarketTool(tool as Tool & { isOwner?: boolean })}
                    >
