...
import { RechargeFlow } from "@/components/payment";
...
export default function BillingPage() {
  ...
  useEffect(() => {
    ...
    const syncPaymentStatus = async () => {
      ...
      try {
        ...
        if (response.code === 200) {
          ...
          if (status.status === "PAID") {
            window.history.replaceState({}, "", window.location.pathname);
            handleRechargeSuccess(orderNo, Number(status.amount || 0));
            return;
          }
      ...
      if (attempts < 20) {
        timer = setTimeout(syncPaymentStatus, 3000);
      } else {
        toast({
          title: "支付状态同步中",
          description: "暂未确认到账，请稍后刷新账单页查看",
          variant: "default"
        });
      }
    };
