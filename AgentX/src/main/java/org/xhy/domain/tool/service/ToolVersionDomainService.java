...
public class ToolVersionDomainService {
    ...
    public ToolVersionDomainService(ToolVersionRepository toolVersionRepository, ToolRepository toolRepository) {
    ...
    public Page<ToolVersionEntity> listToolVersion(QueryToolRequest queryToolRequest) {
        ...
        List<ToolVersionEntity> allPublicList = filterVersionsWithActiveSource(toolVersionRepository.selectList(wrapper));
    ...
    public ToolVersionEntity getToolVersion(String toolId, String version, String userId) {
        ...
        ensureToolSourceExists(toolVersionEntity.getToolId());
    ...
    public int deleteToolVersionsByToolId(String toolId, String userId) {
        Wrapper<ToolVersionEntity> wrapper = Wrappers.<ToolVersionEntity>lambdaQuery()
                .eq(ToolVersionEntity::getToolId, toolId).eq(ToolVersionEntity::getUserId, userId);
        return toolVersionRepository.delete(wrapper);
    }

    private List<ToolVersionEntity> filterVersionsWithActiveSource(List<ToolVersionEntity> versions) {
        if (versions == null || versions.isEmpty()) {
            return new ArrayList<>();
        }

        Set<String> activeToolIds = getActiveToolIds(versions.stream().map(ToolVersionEntity::getToolId).toList());
        return versions.stream().filter(version -> activeToolIds.contains(version.getToolId())).toList();
    }

    private void ensureToolSourceExists(String toolId) {
        if (toolId == null || !getActiveToolIds(List.of(toolId)).contains(toolId)) {
            throw new BusinessException("Tool source has been deleted");
        }
    }

    private Set<String> getActiveToolIds(List<String> toolIds) {
        if (toolIds == null || toolIds.isEmpty()) {
            return new HashSet<>();
        }

        List<String> distinctToolIds = toolIds.stream().filter(Objects::nonNull).distinct().toList();
        if (distinctToolIds.isEmpty()) {
            return new HashSet<>();
        }

        List<ToolEntity> tools = toolRepository.selectByIds(distinctToolIds);
        return tools.stream().map(ToolEntity::getId).collect(Collectors.toSet());
    }
}
