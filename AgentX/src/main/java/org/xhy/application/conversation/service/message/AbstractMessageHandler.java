...
import org.xhy.application.conversation.service.message.attachment.ChatAttachmentContentService;
...
public abstract class AbstractMessageHandler {
    ...
    @Autowired
    protected ChatAttachmentContentService chatAttachmentContentService;
    // 无需事件或单独服务，直接调用异步方法
    // 记忆注入常量（默认开启）
    ...
    protected <T> void processSyncChat(ChatContext chatContext, T connection, MessageTransport<T> transport,
        ...
        try {
            ...
            String finalContent = responseText(chatResponse);
            ...
            // 8. 发送完整响应
            AgentChatResponse response = new AgentChatResponse(finalContent, true);
            ...
            // 10. 执行模型调用计费
            performBillingWithErrorHandling(chatContext, inputTokenCount(chatResponse), outputTokenCount(chatResponse),
    ...
    protected <T> void processChat(Agent agent, T connection, MessageTransport<T> transport, ChatContext chatContext,
        ...
        tokenStream.onCompleteResponse(chatResponse -> {
            String streamedContent = messageBuilder.get().toString();
            String finalContent = responseText(chatResponse, streamedContent);
            ...
            // 发送结束消息
            if (StringUtils.isBlank(streamedContent) && StringUtils.isNotBlank(finalContent)) {
                transport.sendMessage(connection, AgentChatResponse.build(finalContent, MessageType.TEXT));
            }
    ...
    protected void setMessageTokenCount(List<MessageEntity> historyMessages, MessageEntity userEntity,
            MessageEntity llmEntity, ChatResponse chatResponse) {
        setMessageTokenCount(historyMessages, userEntity, llmEntity, chatResponse, null);
    }

    protected void setMessageTokenCount(List<MessageEntity> historyMessages, MessageEntity userEntity,
            MessageEntity llmEntity, ChatResponse chatResponse, String fallbackContent) {
        int outputTokens = outputTokenCount(chatResponse);
        int inputTokens = inputTokenCount(chatResponse);

        llmEntity.setTokenCount(outputTokens);
        llmEntity.setBodyTokenCount(outputTokens);
        llmEntity.setContent(responseText(chatResponse, fallbackContent));
        int bodyTokenSum = 0;
        if (CollectionUtil.isNotEmpty(historyMessages)) {
            bodyTokenSum = historyMessages.stream().filter(Objects::nonNull)
                    .mapToInt(message -> message.getBodyTokenCount() != null ? message.getBodyTokenCount() : 0).sum();
        }
        userEntity.setTokenCount(inputTokens);
        userEntity.setBodyTokenCount(inputTokens - bodyTokenSum);
    }

    protected int inputTokenCount(ChatResponse chatResponse) {
        TokenUsage tokenUsage = chatResponse != null ? chatResponse.tokenUsage() : null;
        Integer tokenCount = tokenUsage != null ? tokenUsage.inputTokenCount() : null;
        return tokenCount != null ? tokenCount : 0;
    }

    protected int outputTokenCount(ChatResponse chatResponse) {
        TokenUsage tokenUsage = chatResponse != null ? chatResponse.tokenUsage() : null;
        Integer tokenCount = tokenUsage != null ? tokenUsage.outputTokenCount() : null;
        return tokenCount != null ? tokenCount : 0;
    }

    protected String responseText(ChatResponse chatResponse) {
        return responseText(chatResponse, null);
    }

    protected String responseText(ChatResponse chatResponse, String fallbackContent) {
        String responseText = "";
        if (chatResponse != null && chatResponse.aiMessage() != null) {
            responseText = StringUtils.defaultString(chatResponse.aiMessage().text());
        }
        return StringUtils.isNotBlank(responseText) ? responseText : StringUtils.defaultString(fallbackContent);
    }
    ...
    protected void buildHistoryMessage(ChatContext chatContext, MessageWindowChatMemory memory) {
        ...
        for (MessageEntity messageEntity : messageHistory) {
            ...
            if (messageEntity.isUserMessage()) {
                List<String> fileUrls = Optional.ofNullable(messageEntity.getFileUrls()).orElse(Collections.emptyList());
                for (String fileUrl : fileUrls) {
                    addAttachmentMessage(memory, fileUrl, chatContext.getModel());
                }
    ...
    /** Adds chat attachments to model memory. Images stay multimodal only for vision-capable models. */
    private void addAttachmentMessage(MessageWindowChatMemory memory, String fileUrl, ModelEntity model) {
        if (StringUtils.isBlank(fileUrl) || chatAttachmentContentService == null) {
            return;
        }
        if (chatAttachmentContentService.isImage(fileUrl)) {
            if (supportsVision(model)) {
                memory.add(UserMessage.from(ImageContent.from(fileUrl)));
            } else {
                memory.add(new UserMessage("[Historical image attachment omitted]\nFile: "
                        + attachmentDisplayName(fileUrl)
                        + "\nThe current model does not support vision input, so the image itself was not sent."));
            }
            return;
        }
        chatAttachmentContentService.buildDocumentMessage(fileUrl)
                .ifPresent(documentMessage -> memory.add(new UserMessage(documentMessage)));
    }

    private boolean supportsVision(ModelEntity model) {
        if (model == null) {
            return false;
        }
        String modelText = String.join(" ", StringUtils.defaultString(model.getModelId()),
                StringUtils.defaultString(model.getName()), StringUtils.defaultString(model.getModelEndpoint()))
                .toLowerCase(Locale.ROOT);
        return modelText.contains("vl") || modelText.contains("vision") || modelText.contains("visual")
                || modelText.contains("omni") || modelText.contains("kimi-k") || modelText.contains("gpt-4o")
                || modelText.contains("gemini") || modelText.contains("claude-3");
    }

    private String attachmentDisplayName(String fileUrl) {
        try {
            String path = java.net.URI.create(fileUrl).getPath();
            int slash = path.lastIndexOf('/');
            String fileName = slash >= 0 ? path.substring(slash + 1) : path;
            return StringUtils.defaultIfBlank(fileName, "image");
        } catch (Exception e) {
            return "image";
        }
    }
    ...
    protected void smartRenameSession(ChatContext chatContext) {
        Thread thread = new Thread(() -> {
            ...
            if (isFirstConversation) {
                ...
                String sessionTitle = responseText(chat);
    ...
    protected ModelCallInfo buildModelCallInfo(ChatContext chatContext, ChatResponse chatResponse, long callTime,
        ...
        return ModelCallInfo.builder().modelEndpoint(chatContext.getModel().getModelEndpoint())
                ...
                .inputTokens(inputTokenCount(chatResponse)).outputTokens(outputTokenCount(chatResponse))
                .callTime((int) callTime).success(success)
