...
public class PaymentAppService {
    ...
    @Value("${payment.callback.base-url:http://localhost:8088/api}")
    private String paymentCallbackBaseUrl;

    @Value("${payment.redirect.success-url:http://localhost:3000/settings/billing?payment=success}")
    private String paymentSuccessUrl;

    @Value("${payment.redirect.cancel-url:http://localhost:3000/settings/billing?payment=cancel}")
    ...
    private PaymentRequest buildPaymentRequest(OrderEntity order, RechargeRequest request) {
        ...
        paymentRequest.setNotifyUrl(normalizeBaseUrl(paymentCallbackBaseUrl) + "/payments/callback/" + platformCode);
        paymentRequest.setSuccessUrl(appendQueryParam(paymentSuccessUrl, "orderNo", order.getOrderNo()));
        paymentRequest.setCancelUrl(appendQueryParam(paymentCancelUrl, "orderNo", order.getOrderNo()));
    ...
    private String appendQueryParam(String url, String key, String value) {
        ...
        return url + separator + java.net.URLEncoder.encode(key, java.nio.charset.StandardCharsets.UTF_8) + "="
    ...
    private List<PaymentMethodDTO.PaymentTypeDTO> getSupportedPaymentTypes(PaymentPlatform platform) {
        ...
        switch (platform) {
            ...
            case WECHAT :
                ...
                break;
            case STRIPE :
                types.add(new PaymentMethodDTO.PaymentTypeDTO("WEB", "Stripe Checkout", true));
                break;
            default :
                // 其他平台暂不支持，跳过
                break;
