...
public class ToolAppService {
    ...
    public Page<ToolVersionDTO> marketTools(QueryToolRequest queryToolRequest, String userId) {
        ...
        List<ToolVersionDTO> list = records.stream().map(toolVersionEntity -> {
            ...
            toolVersionDTO.setIsOwner(toolVersionEntity.getUserId().equals(userId));
    ...
    public List<ToolVersionDTO> getRecommendTools(String userId) {
        ...
        List<ToolVersionDTO> toolVersionDTOs = records.stream().map(toolVersionEntity -> {
            ...
            dto.setIsOwner(toolVersionEntity.getUserId().equals(userId));
            return dto;
        }).toList();
    ...
    public void deleteMarketTool(String toolId, String userId) {
        int deletedCount = toolVersionDomainService.deleteToolVersionsByToolId(toolId, userId);
        if (deletedCount == 0) {
            throw new BusinessException("Tool market record does not exist");
        }
    }
