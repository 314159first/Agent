...
public class DockerService {
    ...
    /** Pull the Docker image if this exact tag is not available locally. */
    private void pullImageIfNotExists(String image) {
        try {
            if (isImageAvailable(image)) {
                logger.debug("Docker image already available: {}", image);
                return;
            }

            logger.info("Pulling Docker image: {}", image);
            CustomPullImageResultCallback callback = new CustomPullImageResultCallback();
            ...
            try {
                ...
                Thread.currentThread().interrupt();
                throw new BusinessException("Docker image pull interrupted: " + image, e);
            }
            callback.awaitSuccess();

            InspectImageResponse imageInfo = inspectImageOrThrow(image);
            logger.info("Docker image ready: {}, id={}", image, imageInfo.getId());
        } catch (BusinessException e) {
            throw e;
        } catch (Exception e) {
            logger.error("Docker image pull failed: {}", image, e);
            throw new BusinessException("Docker image pull failed: " + image + " - " + e.getMessage(), e);
        }
    }
    ...
    private boolean isImageAvailable(String image) {
        try {
            ...
            return imageInfo != null && imageInfo.getId() != null;
        } catch (NotFoundException e) {
            return false;
        } catch (DockerException e) {
            logger.error("Failed to inspect Docker image: {}", image, e);
            throw new BusinessException("Failed to inspect Docker image: " + image + " - " + e.getMessage(), e);
        }
    }

    private InspectImageResponse inspectImageOrThrow(String image) {
        try {
            InspectImageResponse imageInfo = dockerClient.inspectImageCmd(image).exec();
            if (imageInfo == null || imageInfo.getId() == null) {
                throw new BusinessException("Docker image is unavailable after pull: " + image);
            }
            return imageInfo;
        } catch (NotFoundException e) {
            throw new BusinessException("Docker image is unavailable after pull: " + image, e);
        }
    }
