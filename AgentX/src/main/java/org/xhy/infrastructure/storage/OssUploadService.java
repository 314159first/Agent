...
public class OssUploadService {
    ...
    private void validateUploadConfig() {
        ...
        if (isBlank(ossProperties.getBucketName())) {
            missing.add("oss.bucket-name/S3_BUCKET_NAME");
        }
        if (isBlank(ossProperties.getEndpoint())) {
            missing.add("oss.endpoint/S3_ENDPOINT");
        }
        if (isBlank(ossProperties.getAccessKey())) {
            missing.add("oss.access-key/S3_SECRET_ID");
        }
        if (isBlank(ossProperties.getSecretKey())) {
            missing.add("oss.secret-key/S3_SECRET_KEY");
        }
        if (!missing.isEmpty()) {
            throw new BusinessException(
                    "Object storage upload is not configured. Missing: " + String.join(", ", missing));
        }
    }
