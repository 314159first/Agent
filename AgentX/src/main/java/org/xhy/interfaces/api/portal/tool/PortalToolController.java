...
public class PortalToolController {
    ...
    public Result market(QueryToolRequest queryToolRequest) {
        String userId = UserContext.getCurrentUserId();
        return Result.success(toolAppService.marketTools(queryToolRequest, userId));
    }

    @DeleteMapping("/market/{toolId}")
    public Result deleteMarketTool(@PathVariable String toolId) {
        String userId = UserContext.getCurrentUserId();
        toolAppService.deleteMarketTool(toolId, userId);
    ...
    public Result<List<ToolVersionDTO>> getRecommendTools() {
        String userId = UserContext.getCurrentUserId();
        return Result.success(toolAppService.getRecommendTools(userId));
    }
